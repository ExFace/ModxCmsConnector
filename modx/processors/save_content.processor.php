<?php
// sfl: angepasst aus ModX Evolution 1.2.1 save_content.processor.php
	...
	elseif ($alias && !$allow_duplicate_alias) {
		...
		if ($docid > 0) {
			if ($actionToTake == 'edit') {
				throw new Exception(sprintf($_lang["duplicate_alias_found"], $docid, $alias));
			} else {
				throw new Exception(sprintf($_lang["duplicate_alias_found"], $docid, $alias));
			}
		}
...
if($_SESSION['mgrRole'] != 1 && is_array($document_groups)) {
	...
	if(!empty($document_group_list)) {
		...
		if($count == 0) {
			if ($actionToTake == 'edit') {
				throw new Exception(sprintf($_lang["resource_permissions_error"]));
			} else {
				throw new Exception(sprintf($_lang["resource_permissions_error"]));
			}
		}
...
if ($actionToTake != "new") {
	...
	if (!$existingDocument) {
		throw new Exception($_lang["error_no_results"]);
	}
...
if ($use_udperms == 1) {
	if ($existingDocument['parent'] != $parent) {
		...
		if (!$udperms->checkPermissions()) {
			if ($actionToTake == 'edit') {
				throw new Exception(sprintf($_lang['access_permission_parent_denied'], $docid, $alias));
			} else {
				throw new Exception(sprintf($_lang['access_permission_parent_denied'], $docid, $alias));
			}
		}
...
// -- Save META Keywords --
if (!function_exists('saveMETAKeywords')) {
function saveMETAKeywords($id, $keywords, $metatags, $tbl_keyword_xref, $tbl_site_content, $tbl_site_content_metatags) {
	global $modx;
	       //, $keywords, $metatags,
	       //$tbl_keyword_xref,
	       //$tbl_site_content,
...
switch ($actionToTake) {
	case 'new' :
		...
		// save META Keywords
		saveMETAKeywords($key, $keywords, $metatags, $tbl_keyword_xref, $tbl_site_content, $tbl_site_content_metatags);
		...
		include_once __DIR__ . DIRECTORY_SEPARATOR . 'secure_web_documents.inc.php';
		secureWebDocument($key);

		// secure manager documents - flag as private
		include_once __DIR__ . DIRECTORY_SEPARATOR . 'secure_mgr_documents.inc.php';
	...
	case 'edit' :
		...
		if ($id == $site_start && $published == 0) {
			throw new Exception("Document is linked to site_start variable and cannot be unpublished!");
		}
		$today = time();
		if ($id == $site_start && ($pub_date > $today || $unpub_date != "0")) {
			throw new Exception("Document is linked to site_start variable and cannot have publish or unpublish dates set!");
		}
		if ($parent == $id) {
			throw new Exception("Document can not be it's own parent!");
		}
		...
		// save META Keywords
		saveMETAKeywords($id, $keywords, $metatags, $tbl_keyword_xref, $tbl_site_content, $tbl_site_content_metatags);
		...
		break;
	default :
		throw new Exception("No operation set in request.");
}
