...
switch ($modx->event->name) {
	...
	case "OnManagerSaveUser":
		$sql = "SELECT mu.username,mu.password,ma.*  
				...
				WHERE mu.id = '$userid'";
		$rs = $modx->dbQuery($sql);
		if(!$rs) $e->alert("Error while reading database ".mysql_error());
		else {
	...
	*/
	case "OnStripAlias": 
		if ($_REQUEST['alias'] === ''){
			...
			if ($exface->cms()->is_ui_page($_REQUEST['ta'], $_REQUEST['id'])){
				try {
					$app_alias = UiPageFactory::create_from_string($exface->ui(), $_REQUEST['id'], $_REQUEST['ta'])->get_widget_root()->get_meta_object()->get_namespace();
				} catch (Throwable $e){
					break;
				}
...
function exfUserUpdate($mngr) {
	...
	if($count>0) {
		...
		$fields = array();
		$allowed = array("fullname", "email", "phone", "mobilephone", "blocked", "blockeduntil", "dob", "gender", "country", "state", "zip", "fax", "photo", "comment", "blockedafter");
		...
		$modx->updIntTableRow(array("password"=>$mngr["password"]),"web_users"," id='".$web["id"]."'");
	}
	else {
		// create new account
		$fields = array();
		...
		$modx->putIntTableRow(array("username"=>$mngr["username"],"password"=>$mngr["password"]),"web_users");
